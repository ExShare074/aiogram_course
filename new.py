...
from config import TOKEN, WEATHER_CITY, WEATHER_API_KEY
...
async def age(message: Message, state: FSMContext):
    ...
    await message.answer("С какого ты города?")
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(f"http://api.openweathermap.org/data/2.5/weather?q={user_data['city']}&appid={WEATHER_API_KEY}&units=metric&lang=ru") as response:
            if response.status == 200:
                ...
                weather_message = (f"Город - {user_data['city']}\n"
                                    f"Температура - {temprature}°C\n"
